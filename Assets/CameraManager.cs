...
public class CameraManager : MonoBehaviour{
    ...
    private void LateUpdate(){
        ...
        if (_camMode == CamMode.CenterPlayer){
            ...
            List<DummyTower> closestTowers = gm.lm.tm.towers;
            if (closestTowers.Count > 2){
                ...
                if (gm.lm.chm.IsPlayerInsideHull()){
                    Vector3 centroid = gm.lm.chm.GetConvexHullCentroid().transform.position;
    ...
    public List<DummyTower> GetClosestXTowersToPlayer(int x){
        List<DummyTower> closestTowers = new List<DummyTower>();
        if (gm.lm.tm.GetTowers().Count > x){
            // Iterate over the objects
            foreach (DummyTower tower in gm.lm.tm.GetTowers()){
                ...
                if (closestTowers.Count < x){
                    ...
                    DummyTower farthestElement = closestTowers[0];
            ...
            closestTowers = gm.lm.tm.GetTowers();
        }
    ...
    private Bounds CalculateBounds(List<DummyTower> objectsInBounds)
    {
        Bounds bounds = new Bounds(objectsInBounds[0].transform.position, Vector3.zero);
        foreach (DummyTower tower in objectsInBounds)
        {
            Renderer renderer = tower.GetComponent<Renderer>();
            bounds.Encapsulate(renderer.bounds);
            float attackRadius = tower.GetAttackRadius();
    ...
    private Bounds AddPlayerBounds(Bounds bounds,PlayerMovement pm){
        Renderer renderer = pm.sr;
        bounds.Encapsulate(renderer.bounds);
        float viewDistance = pm.viewDistance;
    ...
    void HandleMinimapCam(){
        if (gm.lm.chm.GetConvexHullCentroid() != null){
            Vector3 centroidPos = gm.lm.chm.GetConvexHullCentroid().transform.position;
